  ...
  fetch("./chat.txt")
    .then((response) => {
      response.text().then((data) => {
        let dm = {
          msg: `19/03/21, 7:21 pm - Nitin Kumar CSE: Okay
            19/03/21, 7:22 pm - Krishna Sharma CSE: Then send it
            19/03/21, 7:22 pm - Srishti CSE: On Wednesday
            19/03/21, 7:22 pm - Srishti CSE: On that same paper
            19/03/21, 7:22 pm - Nitin Kumar CSE: Then send it
            `,
        };
        // Extract text from the string
        // let text = dm.msg.match(/(\s\w+)*(:)(\s\w+)*(\n)/g);
        // console.log(text);
      });
    })
    .catch((error) => {
      ...
      function (e) {
        ...
        if (fileTobeRead.type.match(fileExtension)) {
          ...
          fileReader.onload = function (e) {
            ...
            let text = fileReader.result.match(/(\s\w+)*(:)(\s\w+)*(\n)/g);
            // (\s\w+)*(:)(\s\w+)*(.)*(\n)*   New RegExp
            // console.log(text);
            // console.log(sender);
            var count = {};
            var countEmoji = {};
            var datetime = fileReader.result.match(
              /(\d+[\/.]\d+[\/.]\d+)([^-\]]*)/g
            );
            ...
            var emoji = fileReader.result.match(
            ...
            text.forEach((element) => {
              // console.log(element);
              element = element.replace(":", "columtttt");
              // console.log(element);
              const tempData = {
                name: element.split("columtttt")[0],
                count: element.split("columtttt")[1].trim().split(" ").length,
                message: element.split("columtttt")[1],
              };
              if (!textCount[tempData.name]) {
                console.log(element);
                if (element.split("columtttt").length <= 1) {
                  return;
                }
              }
              if (element.split("columtttt").length > 1) {
                textCount[tempData.name] =
                  (textCount[tempData.name] || 0) + tempData.count;
              }
            });
            ...
            plotWordSenderGraph(textCount, "");
            plotSenderGraph(count, "1");
            plotTimeGraph(time);
            if (emoji !== null) {
...
function plotWordSenderGraph(count, id) {
  var senderName = [];
  var senderCount = [];
  var total = 0;
  ...
  totalText = document.getElementById("totalText" + id);
  totalText.innerHTML = '<div class="high">' + total + "</div>Words Were Sent";
  var i = senderCount.indexOf(Math.max(...senderCount));
  senderText = document.getElementById("senderText" + id);
  senderText.innerHTML =
    '<div class="high">' +
    senderCount[i] +
    "</div>Words Were Sent By " +
    senderName[i];
  var ctx = document.getElementById("senderChart" + id).getContext("2d");
...
function plotSenderGraph(count, id) {
  var senderName = [];
  var senderCount = [];
  var total = 0;
  for (var key in count) {
  ...
  totalText = document.getElementById("totalText" + id);
  totalText.innerHTML =
    '<div class="high">' + total + "</div>Messages Were Sent";
  var i = senderCount.indexOf(Math.max(...senderCount));
  senderText = document.getElementById("senderText" + id);
  senderText.innerHTML =
    '<div class="high">' +
    senderCount[i] +
    "</div>Messages Were Sent By " +
    senderName[i];
  var ctx = document.getElementById("senderChart" + id).getContext("2d");
  var myChart = new Chart(ctx, {
    type: "pie",
...
function plotTimeGraph(time) {
  ...
  timeText.innerHTML =
    '<div class="high">' +
    time[i] +
    "</div> Messages Were Sent Between " +
    interval[i];
...
function downloadURI(uri, name) {
  ...
  var text = document.createTextNode(
    "Click On The Link If Download Doesn't Start Automatically"
  );
...
function printToFile() {
  document.getElementById("foot-cont").innerHTML =
    '<div class="flow-text" style="margin-top:30px">Generated Using WhatsApp Stat <div style="color:teal">whatsappstat.developerpritam.in/</div></div>';
